...
window.Sparring = (function () {
   ...
   * 所以数量≠话术总数：48条里只有20多条是客户台词，其余是方法类（报价锚定/复盘），不适合扮演。
   * 优先读Store（含用户自建），SEED只做兜底——否则用户自己加的「客户说…」永远练不到。 */
      ...
      <div class="card">
        ...
        <div class="field-row">
          <div class="field"><label>场景（话术库${totalScripts}条，其中${list.length}个适合对练）</label>
            <select id="spar-scene">${opts || '<option value="">话术库是空的</option>'}</select><div class="hint">只挑标题像客户原话的（「客户说…」「对方…」），方法类话术不适合扮演，所以数量比话术总数少是正常的。</div></div>
