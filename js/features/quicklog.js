...
window.QuickLog = (function () {
  ...
  /* ============================================================
   * 客户匹配：全名优先，其次简称，最后姓+职务（王总/陈经理/马主任）
   * 第三轮是浏览器实测补的：销售口头都是「王总」，库里是「王建国」，
   * 前两轮都认不出，一句话录入每天都要手动选人。
   * ============================================================ */
  ...
  function matchCustomer(text) {
    ...
    // 第三轮：姓+职务（「王总」「陈经理」「马主任」→ 王建国/陈敏/马主任）
    const honorHits = [];
    list.forEach(c => {
    ...
    if (honorHits.length > 1) {
      ...
      // 同姓多人时，若文本同时出现某家公司简称，就定到那家（「恒力王总」→ 恒力那家）
