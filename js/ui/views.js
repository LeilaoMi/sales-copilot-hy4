...
window.Views = (function () {
   ...
   * 2. 客户库
   * ============================================================ */
  /* 客户搜索别名：让「王总」能搜到王建国（姓+职务）。
   * 一句话录入已支持姓+职务匹配，搜索不同步就会出现「录入能认出、搜索搜不到」的割裂。 */
  const CUST_HONORS = ['董事长', '经理', '主任', '厂长', '店长', '先生', '女士', '小姐', '老板', '老师', '总', '姐', '哥', '董', '工'];
  const CUST_COMPOUND = ['欧阳', '司马', '上官', '诸葛', '东方', '皇甫', '尉迟', '公孙', '轩辕', '令狐', '钟离', '宇文', '长孙', '慕容', '司徒', '司空'];
  function custSurname(contact) {
    const c = String(contact || '').trim();
    if (!c) return '';
    if (c.length >= 2 && CUST_COMPOUND.indexOf(c.slice(0, 2)) >= 0) return c.slice(0, 2);
    return c.slice(0, 1);
  }
  function custSearchText(c) {
    const base = [c.name, c.contact, c.phone, c.wechat, c.industry, c.note, c.tags].join(' ');
    const sn = custSurname(c.contact);
    if (!sn) return base;
    return base + ' ' + CUST_HONORS.map(h => sn + h).join(' ');
  }
    ...
    if (!configured) {
      ...
      <div class="card">
        ...
        <div class="hint" id="cloud-hint">没有 Supabase 项目？它是免费的，注册后建个项目，
          把项目根目录 <code>db/supabase.sql</code> 整段粘进 SQL Editor 执行一次就行，不用管服务器。</div>
      </div>`;
    }
      ...
      <details class="sync-guide">
        ...
        <div class="small">
          ...
          <p><b>② Supabase 免费版 · 3 分钟，长期用推荐这条</b><br>
            ...
            <code>db/supabase.sql</code> → 回来选「Supabase / 云开发 REST」，填 Project URL 和 anon key。
