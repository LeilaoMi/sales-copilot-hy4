 ...
 * 两种产物，二选一：
 *
 *   node tools/build.js          单文件 HTML（双击可开、可微信发给同事）
 *   node tools/build.js --pages  生成 public/ 目录，用于 Cloudflare Pages / 静态托管
 *
 * 为什么不直接把整个项目目录丢给托管平台：
...
const dir = __dirname;
const root = path.join(dir, '..'); // tools/ -> 项目根
...
if (process.argv.includes('--pages')) {
   ...
   * 「缓存规则静默失效」的站点。 */
  const headersSrc = path.join(root, 'deploy', '_headers');
  ...
  if (fs.existsSync(swSrc)) {
    ...
    const statics = ['./', './index.html', './assets/css/styles.css',
                     './manifest.json', './assets/icons/icon-192.png', './assets/icons/icon-512.png', './assets/icons/apple-touch-icon.png'];
...
/* ---------- 默认：单文件 HTML ---------- */
const out = path.join(root, '..', '销冠助手-单文件版.html');
